...
const timeframeOptions = ['This week', 'Within 2 weeks', 'This month', 'Planning ahead'];
...
const nextSteps = [
  {
    ...
    description: 'Our team looks at your request so we can prepare a relevant walkthrough.',
  },
  {
  ...
  },
  {
    ...
    description: 'The session is tailored around your workflow, content, and operational priorities.',
  },
  {
    title: 'We recommend next steps if relevant',
    description: 'That may include a pilot, rollout path, or pricing discussion based on your needs.',
  },
];
...
const teams = [
  {
    ...
    description: 'Centralize multilingual workflows and scale translation programs across regions and business units.',
    icon: Globe2,
  },
  {
    title: 'Product and Engineering Teams',
    description: 'Support software releases, digital products, and multilingual user experiences with connected workflows.',
    icon: Code2,
  },
  {
    title: 'Legal and Compliance Teams',
    description: 'Review secure workflows for contracts, policies, regulatory content, and audit-sensitive documents.',
    icon: FileCheck2,
  },
  {
    title: 'Life Sciences and Clinical Teams',
    description: 'Support controlled workflows for clinical, regulatory, medical, and patient-facing content.',
    icon: Stethoscope,
  },
  {
    title: 'Marketing and Ecommerce Teams',
    description: 'Move faster on campaigns, websites, product content, and brand messaging across languages.',
    icon: Briefcase,
  },
  {
    title: 'Procurement and Vendor Management Teams',
    description: 'Gain visibility, process consistency, and scalable enterprise translation operations.',
    icon: Building2,
  },
];
...
function TextInput({
  ...
  return (
    <div>
      <label htmlFor={name} className="mb-2 block text-[14px] font-semibold text-[#344054]">
        {label}
      </label>
      <div
        className={`flex items-center gap-3 rounded-[16px] border bg-white px-4 py-3.5 shadow-[0_1px_2px_rgba(16,24,40,0.04)] transition ${
          error ? 'border-[#FDA29B] ring-2 ring-[#FEE4E2]' : 'border-[#E5E7EB] focus-within:border-[#C11D63] focus-within:ring-2 focus-within:ring-[#F7D9E7]'
        }`}
      >
    ...
    <div>
      ...
      >
        <select
          ...
          value={value}
          onChange={(e) => onChange(name, e.target.value)}
          className={`w-full border-0 bg-transparent p-0 text-[15px] focus:outline-none ${value ? 'text-[#101828]' : 'text-[#98A2B3]'}`}
        >
    ...
    >
      <span
        className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-[4px] border ${
          selected ? 'border-[#C11D63] bg-[#C11D63]' : 'border-[#D0D5DD] bg-white'
        }`}
      >
...
export default function StepesDemoRequestWireframe() {
  ...
  const [submittedRequest, setSubmittedRequest] = useState<FormData | null>(null);
  const [errors, setErrors] = useState<FormErrors>({});
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [submitCount, setSubmitCount] = useState(0);
  const editorRef = useRef<HTMLDivElement | null>(null);

  const plainMessage = useMemo(() => getPlainTextFromHtml(formData.messageHtml), [formData.messageHtml]);
  ...
  useEffect(() => {
    if (editorRef.current && editorRef.current.innerHTML !== formData.messageHtml) {
  ...
  const toggleContentType = (item: string) => {
    setFormData((prev) => {
      ...
      return {
        ...prev,
        contentType: exists ? prev.contentType.filter((type) => type !== item) : [...prev.contentType, item],
      };
    });
  ...
  const validateForm = () => {
    ...
    if (!formData.workEmail.trim()) nextErrors.workEmail = 'Please enter your work email.';
    else if (!hasValidEmail(formData.workEmail)) nextErrors.workEmail = 'Please enter a valid email address.';

    if (!formData.name.trim()) nextErrors.name = 'Please enter your name.';
    if (!formData.company.trim()) nextErrors.company = 'Please enter your company name.';
    if (!formData.jobTitle.trim()) nextErrors.jobTitle = 'Please enter your job title.';
    if (formData.topics.length === 0) nextErrors.topics = 'Please select at least one demo topic.';
    if (formData.contentType.length === 0) nextErrors.contentType = 'Please select at least one content type.';
    if (!formData.preferredTimeframe.trim()) nextErrors.preferredTimeframe = 'Please select a preferred demo timeframe.';
    if (!plainMessage.trim()) nextErrors.messageHtml = 'Please share a little about your workflow or goals.';
        ...
        <section className="py-4 lg:py-6">
          ...
          >
            ...
            <p className="mx-auto mt-5 max-w-[760px] text-[17px] leading-7 text-[#475467] sm:text-[18px] sm:leading-8">
              Complete the form below to request a tailored walkthrough of Stepes for enterprise translation, AI + human workflows, quality, security, and scale.
            </p>
          </motion.div>
          ...
          >
            ...
            {isSubmitted && submittedRequest ? (
                ...
                <div className="rounded-[28px] border border-[#CDEFD9] bg-[#F6FEF9] p-6 sm:p-8">
                  ...
                  <div className="mx-auto mt-5 max-w-[760px] text-center">
                    ...
                    <p className="mt-4 text-[16px] leading-7 text-[#475467]">
                      A Stepes specialist will review your request and follow up to confirm fit and timing. We typically respond within 1 business day.
                    </p>
                  </div>
                  ...
                  <div className="mt-8 grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
                    <div className="rounded-[22px] border border-[#DCEEE4] bg-white p-5">
                      ...
                      <div className="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                          <div className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#98A2B3]">Name</div>
                          <div className="mt-1 text-[15px] text-[#101828]">{submittedRequest.name}</div>
                        </div>
                        <div>
                          <div className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#98A2B3]">Company</div>
                          <div className="mt-1 text-[15px] text-[#101828]">{submittedRequest.company}</div>
                        </div>
                        <div>
                          <div className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#98A2B3]">Work Email</div>
                          <div className="mt-1 text-[15px] text-[#101828]">{submittedRequest.workEmail}</div>
                        </div>
                        <div>
                          <div className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#98A2B3]">Timeframe</div>
                          <div className="mt-1 text-[15px] text-[#101828]">{submittedRequest.preferredTimeframe}</div>
                        </div>
                      </div>

                      <div className="mt-5">
                        <div className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#98A2B3]">Requested Topics</div>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {submittedRequest.topics.map((item) => (
                            <span key={item} className="rounded-full border border-[#DCEEE4] bg-[#F6FEF9] px-3 py-1.5 text-[13px] font-medium text-[#027A48]">
                              {item}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="mt-5">
                        <div className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#98A2B3]">Content Types</div>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {submittedRequest.contentType.map((item) => (
                            <span key={item} className="rounded-full border border-[#E5E7EB] bg-[#FCFCFD] px-3 py-1.5 text-[13px] font-medium text-[#475467]">
                              {item}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="mt-4 space-y-4">
                        {nextSteps.map((step, index) => (
                          <div key={step.title} className="flex gap-3">
                            ...
                            <div>
                              <div className="text-[15px] font-semibold text-[#101828]">{step.title}</div>
                              <div className="mt-1 text-[14px] leading-6 text-[#667085]">{step.description}</div>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <form onSubmit={handleSubmit} className="mt-6">
                ...
                <div className="mt-6">
                  <label className="mb-3 block text-[14px] font-semibold text-[#344054]">What would you like to see?</label>
                ...
                <div className="mt-6">
                  <label className="mb-3 block text-[14px] font-semibold text-[#344054]">Content Type</label>
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="mt-6">
                  ...
                  <div
                    className={`rounded-[16px] border bg-white shadow-[0_1px_2px_rgba(16,24,40,0.04)] transition ${
                      errors.messageHtml ? 'border-[#FDA29B] ring-2 ring-[#FEE4E2]' : 'border-[#E5E7EB] focus-within:border-[#C11D63] focus-within:ring-2 focus-within:ring-[#F7D9E7]'
                    }`}
                  >
                ...
                <div className="mt-6 rounded-[20px] border border-[#ECECF2] bg-[#FCFCFD] p-4 sm:p-5">
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    {trustSignals.map((item) => (
                      <div key={item} className="flex items-center gap-3 rounded-[14px] bg-white px-3 py-3 ring-1 ring-[#ECECF2]">
                        <div className="h-2 w-2 shrink-0 rounded-full bg-[#C11D63]" />
                        <div className="text-[14px] leading-[1.45] text-[#475467]">{item}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="mt-6 flex flex-col gap-4 border-t border-[#ECECF2] pt-6 sm:flex-row sm:items-center sm:justify-between">
                  <div className="max-w-[480px] lg:max-w-[440px]">
                    <p className="text-[14px] leading-6 text-[#667085]">
                      A Stepes specialist will review your request and follow up to schedule a tailored platform walkthrough for your team.
                    </p>
            ...
            <div className="mt-10 grid gap-5 lg:grid-cols-4">
              {nextSteps.map((step, index) => (
                ...
                >
                  ...
                  <h3 className="mt-4 text-[18px] font-semibold tracking-[-0.03em] text-[#101828]">{step.title}</h3>
                  <p className="mt-3 text-[15px] leading-7 text-[#667085]">{step.description}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section className="border-t border-[#ECECF2] py-14 lg:py-16">
          <div className="mx-auto max-w-[1200px]">
            <div className="mx-auto max-w-[980px] text-center">
              ...
              <h2 className="mt-5 text-[32px] font-semibold tracking-[-0.04em] text-[#0F172A] sm:text-[38px] lg:text-[40px]">
                We can tailor the demo around the teams and workflows most relevant to your organization
              </h2>
            </div>
              ...
              {teams.map(({ title, description, icon: Icon }, index) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-[15px] leading-7 text-[#667085]">{description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <section className="py-12 lg:py-14">
          <div className="mx-auto max-w-[900px] text-center">
            <p className="text-[15px] leading-7 text-[#667085]">
              Need to share anything else before the session? Your Stepes contact can help refine the agenda, confirm timing, and prepare the most relevant walkthrough for your team.
            </p>
          </div>
        </section>
      )}
